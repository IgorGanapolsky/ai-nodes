...
async function displayResults(stats, connectorMetrics, statementPath) {
  ...
  for (const [network, metrics] of connectorMetrics) {
    ...
    console.log(
      chalk.white(`  CPU Usage: ${chalk.cyan(metrics.resource_utilization.cpu.toFixed(1))}%`),
    );
    console.log(
      chalk.white(`  Memory Usage: ${chalk.cyan(metrics.resource_utilization.memory.toFixed(1))}%`),
    );
    if (metrics.resource_utilization.gpu) {
      console.log(
        chalk.white(`  GPU Usage: ${chalk.cyan(metrics.resource_utilization.gpu.toFixed(1))}%`),
      );
    }
    console.log(
      chalk.white(`  Daily Earnings: ${chalk.green('$' + metrics.earnings.daily.toFixed(2))}`),
    );
  }
  // Last 7 Days Summary
  console.log(chalk.blue.bold('\n📊 Last 7 Days Summary'));
