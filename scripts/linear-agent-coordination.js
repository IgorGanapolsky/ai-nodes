...
function defaultState() {
  return {
    ...
    workflow: {
      ...
      createdCoordinationIssueTitle: null,
      createdCoordinationIssueUrl: null,
      agentTasks: {},
  ...
  async createAgentTask(agentName, taskTitle, taskDescription, priority = 0) {
    try {
      ...
      if (!this.state.workflow.agentTasks[agentName]) {
        this.state.workflow.agentTasks[agentName] = {
          created: true,
          issueId: task.id,
          title: task.title,
          url: task.url,
        ...
        this.state.workflow.agentTasks[agentName].title = task.title;
        this.state.workflow.agentTasks[agentName].url = task.url;
      }
      saveState(this.state);
...
- Progress updates in this main coordination issue`,
      ...
      this.state.workflow.createdCoordinationIssueTitle = issue.title || 'Coordination Issue';
      this.state.workflow.createdCoordinationIssueUrl = issue.url;
  ...
  /**
   * Revenue loop (one-shot):
   * - Find 'opportunity' issues
   * - Score/Sort (by priority desc)
   * - Assign 'Outreach' agent tasks to least busy agent
   */
  async revenueLoopOnce({ limit = 50 } = {}) {
    const linearService = await getLinearService();
    const issues = await linearService.listIssues({ limit });
    ...
    for (const opp of opportunities) {
      ...
      for (const [, agent] of this.agents) {
        if (!chosen || agent.activeTasks.length < chosen.activeTasks.length) chosen = agent;
      }
      if (!chosen) break;
      ...
      created += 1;
      console.log(`💼 Created outreach task for ${chosen.name} from opportunity ${opp.id}`);
    }
    console.log(
      `✅ Revenue loop processed ${opportunities.length} opportunities; created ${created} tasks.`,
    );
  ...
  try {
    switch (command) {
      ...
      case 'create-task':
        if (!LINEAR_API_KEY) {
          console.error(
            '❌ LINEAR_API_KEY is required for this command. Try `status` or set the env var.',
          );
          process.exit(1);
        }
        ...
        await coordinator.createAgentTask(
          agentName2,
          taskTitle,
          taskDescription,
          parseInt(priority) || 0,
      ...
      case 'revenue-loop':
        ...
        await coordinator.revenueLoopOnce({ limit: parseInt(args[1] || '50') });
        break;
      ...
      case 'status':
        ...
        console.log(
          'Coordination Issue:',
          coordinator.state.workflow.createdCoordinationIssueId || 'not created',
        );
        console.log('Agent Tasks:', coordinator.state.workflow.agentTasks);
        console.log(
          'State File:',
          path.join(process.cwd(), 'logs', 'agents', 'linear-coordination-state.json'),
        );
        break;
...
Commands:
  ...
  revenue-loop [limit]  Process 'opportunity' issues and assign outreach tasks
  ensure-labels  Print recommended labels and settings link
