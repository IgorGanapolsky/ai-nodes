...
export class OwnAIConnector extends BaseConnector {
    ...
    async getEarnings(period, nodeId) {
        ...
        try {
            ...
            const response = await this.makeRequest('GET', `${OwnAIConnector.API_ENDPOINTS.EARNINGS}?${params}`);
    ...
    async getMetrics(nodeId) {
        ...
        try {
            const [jobsResponse, utilizationResponse] = await Promise.all([
                this.makeRequest('GET', `${OwnAIConnector.API_ENDPOINTS.JOBS}${nodeId ? `?node_id=${nodeId}` : ''}`),
                this.makeRequest('GET', `${OwnAIConnector.API_ENDPOINTS.UTILIZATION}${nodeId ? `?node_id=${nodeId}` : ''}`)
            ]);
            if (nodeId) {
