...
export class Huddle01Connector extends BaseConnector {
    ...
    async getEarnings(period, nodeId) {
        ...
        try {
            ...
            const response = await this.makeRequest('GET', `${Huddle01Connector.API_ENDPOINTS.REWARDS}?${params}`);
    ...
    async getMetrics(nodeId) {
        ...
        try {
            const [sessionsResponse, bandwidthResponse] = await Promise.all([
                this.makeRequest('GET', `${Huddle01Connector.API_ENDPOINTS.SESSIONS}${nodeId ? `?node_id=${nodeId}` : ''}`),
                this.makeRequest('GET', `${Huddle01Connector.API_ENDPOINTS.BANDWIDTH}${nodeId ? `?node_id=${nodeId}` : ''}`)
            ]);
            if (nodeId) {
