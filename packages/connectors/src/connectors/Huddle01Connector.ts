  ...
  async getMetrics(nodeId?: string): Promise<NodeMetrics | NodeMetrics[]> {
    ...
    try {
      const [sessionsResponse, bandwidthResponse] = await Promise.all([
        this.makeRequest<any>('GET', `${Huddle01Connector.API_ENDPOINTS.SESSIONS}${nodeId ? `?node_id=${nodeId}` : ''}`),
        this.makeRequest<any>('GET', `${Huddle01Connector.API_ENDPOINTS.BANDWIDTH}${nodeId ? `?node_id=${nodeId}` : ''}`)
      ]);
