import { IConnector, DeviceStatus, DeviceMetrics, DeviceOccupancy, PricingSuggestion } from './types';
...
export abstract class BaseConnector implements IConnector {
  ...
  abstract getOccupancy(externalId: string, periodStart: Date, periodEnd: Date): Promise<DeviceOccupancy>;
  abstract suggestPricing(externalId: string, targetUtilization: number): Promise<PricingSuggestion>;
  abstract applyPricing(externalId: string, pricePerHour: number, dryRun: boolean): Promise<boolean>;
   ...
   */
  protected seededRandomBoolean(seed: string, probability: number = 0.5, index: number = 0): boolean {
   ...
   */
  protected generateMockOccupancy(externalId: string, periodStart: Date, periodEnd: Date): DeviceOccupancy {
   ...
   */
  protected generateMockPricingSuggestion(externalId: string, targetUtilization: number): PricingSuggestion {
   ...
   */
  protected async mockApplyPricing(externalId: string, pricePerHour: number, dryRun: boolean): Promise<boolean> {
