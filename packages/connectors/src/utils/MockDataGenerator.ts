...
export class MockDataGenerator {
   ...
   */
  private static generateTransactionDescription(type: string, amount: number): string {
    switch (type) {
      case 'earnings':
   ...
   */
  private static getCurrency(connectorType: ConnectorType): string {
    switch (connectorType) {
      case ConnectorType.IONET:
        return 'IO';
      case ConnectorType.NOSANA:
   ...
   */
  private static hasGpu(connectorType: ConnectorType): boolean {
   ...
   */
  static generatePeriod(type: Period['type'] = 'day'): Period {
    const end = new Date();
    let start: Date;
