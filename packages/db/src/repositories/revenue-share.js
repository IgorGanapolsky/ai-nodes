...
export class RevenueShareRepository extends BaseRepository {
        ...
        for (const config of shareConfigs) {
            if (!nodeEarnings.has(config.nodeId)) {
                ...
                const earningsResult = await this.db
                    ...
                    .from(sql `earnings`)
                    .where(and(eq(sql `node_id`, config.nodeId), sql `timestamp >= ${Math.floor(periodStart.getTime() / 1000)}`, sql `timestamp <= ${Math.floor(periodEnd.getTime() / 1000)}`));
    ...
    async getRevenueShareReport(filters = {}, dateRange) {
        ...
        if (dateRange) {
            ...
            whereConditions.push(and(sql `${this.table.periodStart} >= ${startTimestamp}`, sql `${this.table.periodEnd} <= ${endTimestamp}`));
        }
