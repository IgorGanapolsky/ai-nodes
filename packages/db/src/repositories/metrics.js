...
export class MetricsRepository extends BaseRepository {
    ...
    async getLatestForAllNodes() {
        const result = await this.db
            .select()
            .from(this.table)
            .where(sql `${this.table.timestamp} = (
    ...
    async getTimeSeries(nodeIds, interval, startDate, endDate, metricTypes) {
        ...
        const whereClause = and(or(...nodeIdArray.map(nodeId => eq(this.table.nodeId, nodeId))), gte(this.table.timestamp, startTimestamp), lte(this.table.timestamp, endTimestamp));
    ...
    async getAggregatedMetrics(nodeIds, startDate, endDate) {
        const nodeIdArray = Array.isArray(nodeIds) ? nodeIds : [nodeIds];
        const startTimestamp = Math.floor(startDate.getTime() / 1000);
        const endTimestamp = Math.floor(endDate.getTime() / 1000);
        const whereClause = and(or(...nodeIdArray.map(nodeId => eq(this.table.nodeId, nodeId))), gte(this.table.timestamp, startTimestamp), lte(this.table.timestamp, endTimestamp));
    ...
    async detectPerformanceAlerts(thresholds = {}) {
    ...
    async calculateHealthScores(nodeIds) {
        let nodes;
        if (nodeIds) {
            nodes = nodeIds;
        }
        ...
        for (const nodeId of nodes) {
            ...
            const healthScore = (cpuScore * 0.25 +
                memoryScore * 0.25 +
                storageScore * 0.25 +
                networkScore * 0.15 +
                uptimeScore * 0.10);
    ...
    async getResourceTrends(nodeId, days = 7) {
