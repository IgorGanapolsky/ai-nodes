...
export declare class RevenueShareRepository extends BaseRepository<typeof revenueShares, RevenueShare, NewRevenueShare> {
    protected table: any;
    ...
    findByPeriod(period: string, options?: {
        pagination?: PaginationOptions;
    ...
    createRevenueShare(shareData: Omit<NewRevenueShare, 'id' | 'amount' | 'timestamp' | 'createdAt' | 'updatedAt'>): Promise<RevenueShare>;
    markAsPaid(shareIds: string[], transactionHash?: string): Promise<number>;
    generateRevenueShares(period: string, periodStart: Date, periodEnd: Date, shareConfigs: Array<{
        nodeId: string;
        shareType: string;
        percentage: number;
        recipientId?: string;
    ...
    getRevenueShareReport(filters?: RevenueShareFilters, dateRange?: {
        start: Date;
        end: Date;
    }): Promise<RevenueShareReport>;
    getPendingPayouts(groupBy?: 'shareType' | 'recipient' | 'period'): Promise<Array<{
        group: string;
        count: number;
        totalAmount: number;
        currency: string;
    }>>;
    getRevenueShareProjections(nodeId: string, shareConfigs: Array<{
        shareType: string;
        percentage: number;
    }>, projectionDays?: number): Promise<Array<{
        shareType: string;
        projectedAmount: number;
        percentage: number;
    }>>;
    getRevenueShareHistory(period?: 'month' | 'quarter' | 'year', limit?: number): Promise<Array<{
        period: string;
        totalShares: number;
        totalAmount: number;
        paidAmount: number;
