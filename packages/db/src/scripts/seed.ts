...
function generateNodes(userIds: string[]) {
  const nodeTypes = ['storj', 'filecoin', 'chia', 'akash', 'theta', 'livepeer', 'helium'] as const;
  const statuses = ['active', 'inactive', 'error', 'maintenance', 'pending'] as const;
...
function generateEarnings(nodeIds: string[]) {
...
function generateRevenueShares(nodeIds: string[]) {
