 ...
 */
function getStrategyMultiplier(strategy: PricingStrategy): number {
  switch (strategy) {
    case 'conservative':
      return 0.5;
    case 'aggressive':
 ...
 */
function applyMarketConditions(baseAdjustment: number, conditions: MarketConditions): number {
 ...
 */
export function generatePricingRoadmap(
  currentMetrics: {
    utilization: number;
    price: number;
    queueDepth: number;
