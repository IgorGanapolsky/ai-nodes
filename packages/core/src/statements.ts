 ...
 * Generate CSV header
 */
function getCSVHeader(delimiter: string, currency: string): string {
  const headers = [
    'Date',
    'Node ID',
    'Node Name',
 ...
 */
export function generateStatementSummary(records: StatementRecord[]): StatementSummary {
  if (records.length === 0) {
  ...
  let topPerformingNode: string | undefined;
  let maxRevenue = 0;
