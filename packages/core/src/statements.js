...
export function recordsToCSV(records, options = {}) {
    const { includeHeader = true, dateFormat = 'yyyy-MM-dd', delimiter = ',', decimalPlaces = 2, currency = 'USD', } = options;
    if (records.length === 0) {
...
export function exportStatementSummaryToCSV(summary, options = {}) {
    const { dateFormat = 'yyyy-MM-dd', delimiter = ',', decimalPlaces = 2, currency = 'USD', } = options;
    const lines = [];
    // Add summary header
