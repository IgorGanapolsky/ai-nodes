 ...
 */
export function suggestPriceAdjustment(currentUtilization, targetUtilization, currentPrice, queueDepth = 0, strategy = 'conservative', marketConditions) {
    // Validate inputs
 ...
 */
export function calculateOptimalPrice(currentPrice, currentUtilization, demandElasticity, targetUtilization) {
...
export function generatePricingRoadmap(currentMetrics, targets) {
    ...
    const immediate = suggestPriceAdjustment(currentMetrics.utilization, targets.utilization, currentMetrics.price, currentMetrics.queueDepth, 'aggressive');
    // Short-term (1-4 weeks): Balanced approach
    const shortTermPrice = immediate.suggestedPrice;
    const shortTerm = suggestPriceAdjustment(currentMetrics.utilization, targets.utilization, shortTermPrice, Math.floor(currentMetrics.queueDepth * 0.7), // Assume queue will reduce
    'conservative');
    // Long-term (1-3 months): Revenue optimization
    const longTermPrice = shortTerm.suggestedPrice;
    const longTerm = suggestPriceAdjustment(targets.utilization * 0.9, // Assume we'll be closer to target
    targets.utilization, longTermPrice, 0, // Assume queue will be managed
    'market_based');
    return {
        immediate,
        shortTerm,
