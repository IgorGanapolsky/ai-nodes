...
export interface RollingAverageConfig {
  ...
  weightDecay?: number; // For exponential weighting
}
 ...
 */
export function calculateEWMA(data: DataPoint[], alpha: number): DataPoint[] {
  if (alpha <= 0 || alpha > 1) {
 ...
 */
export function calculateUtilization(activeHours: number, totalHours: number): number {
  if (totalHours <= 0) {
 ...
 */
export function calculateAverageUtilization(metrics: PerformanceMetrics[]): {
  average: number;
  min: number;
  max: number;
 ...
 */
export function calculateRevenuePerHour(metrics: PerformanceMetrics[]): {
  current: number;
  average: number;
  trend: TrendAnalysis;
} {
