...
export function MetricsSummary({ data }) {
    const earningsProgress = data.targetEarnings > 0 ? (data.totalEarnings / data.targetEarnings) * 100 : 0;
    const activeOwnerPercentage = data.totalOwners > 0 ? (data.activeOwners / data.totalOwners) * 100 : 0;
    const deviceOnlinePercentage = data.totalDevices > 0 ? (data.onlineDevices / data.totalDevices) * 100 : 0;
    ...
    const metrics = [
        {
            ...
            badge: activeOwnerPercentage > 80 ? 'Good' : activeOwnerPercentage > 60 ? 'Fair' : 'Needs attention',
            color: activeOwnerPercentage > 80 ? 'text-green-600' : activeOwnerPercentage > 60 ? 'text-yellow-600' : 'text-red-600',
        },
        {
            title: 'Device Status',
            ...
            badge: deviceOnlinePercentage > 90 ? 'Excellent' : deviceOnlinePercentage > 75 ? 'Good' : 'Critical',
            color: deviceOnlinePercentage > 90 ? 'text-green-600' : deviceOnlinePercentage > 75 ? 'text-yellow-600' : 'text-red-600',
        },
        {
            ...
            color: data.averageUptime > 95 ? 'text-green-600' : data.averageUptime > 85 ? 'text-yellow-600' : 'text-red-600',
        },
        {
            ...
            color: earningsProgress >= 95 ? 'text-green-600' : earningsProgress >= 80 ? 'text-yellow-600' : 'text-red-600',
        ...
        },
        {
            ...
            badge: data.performanceScore > 80 ? 'Excellent' : data.performanceScore > 60 ? 'Good' : 'Poor',
            color: data.performanceScore > 80 ? 'text-green-600' : data.performanceScore > 60 ? 'text-yellow-600' : 'text-red-600',
        },
        {
            title: 'Active Alerts',
            value: data.totalAlerts.toString(),
            description: data.criticalAlerts > 0 ? `${data.criticalAlerts} critical alerts` : 'No critical alerts',
            icon: AlertTriangle,
            ...
            color: data.totalAlerts === 0 ? 'text-green-600' : data.criticalAlerts > 0 ? 'text-red-600' : 'text-yellow-600',
        },
    ];
    return (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6">
      {metrics.map((metric, index) => {
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <Badge variant={metric.badge === 'Excellent' || metric.badge === 'On-pace' || metric.badge === 'All Clear'
                    ? 'default'
              ...
              <div className="space-y-1">
                ...
                <div className="flex justify-between items-center text-xs text-muted-foreground">
                  ...
                  {metric.extra && (<span className={metric.trend === 'up' ? 'text-green-600' : 'text-red-600'}>
                      {metric.extra}
                    </span>)}
                </div>
              </div>
