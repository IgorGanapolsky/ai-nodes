...
import { Area, AreaChart, Bar, BarChart, Line, LineChart, ResponsiveContainer, Tooltip, XAxis, YAxis, Legend, ReferenceLine, } from 'recharts';
...
export function UtilizationChart({ ownerId, timeframe, isLoading }) {
    ...
    return (<div className="space-y-4">
      ...
      {/* Charts */}
      <Tabs value={chartType} onValueChange={(value) => setChartType(value)}>
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={data}>
                    ...
                    <Legend />
                    <Area type="monotone" dataKey="targetEarnings" stackId="1" stroke="hsl(var(--muted-foreground))" fill="url(#targetGradient)" name="Target Earnings"/>
                    <Area type="monotone" dataKey="earnings" stackId="2" stroke="hsl(var(--primary))" fill="url(#earningsGradient)" name="Actual Earnings"/>
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
            ...
            <CardContent>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={data}>
                    <XAxis dataKey="date" tick={{ fontSize: 12 }} tickLine={{ stroke: 'hsl(var(--border))' }}/>
                    <YAxis yAxisId="uptime" orientation="left" tick={{ fontSize: 12 }} tickLine={{ stroke: 'hsl(var(--border))' }} domain={[0, 100]}/>
                    <YAxis yAxisId="devices" orientation="right" tick={{ fontSize: 12 }} tickLine={{ stroke: 'hsl(var(--border))' }}/>
                    <Tooltip content={<CustomTooltip />}/>
                    <Legend />
                    <ReferenceLine yAxisId="uptime" y={95} stroke="hsl(var(--destructive))" strokeDasharray="5 5"/>
                    <Line yAxisId="uptime" type="monotone" dataKey="uptime" stroke="hsl(var(--primary))" strokeWidth={2} dot={{ r: 3 }} name="Uptime %"/>
                    <Line yAxisId="devices" type="monotone" dataKey="onlineDevices" stroke="hsl(var(--success))" strokeWidth={2} dot={{ r: 3 }} name="Online Devices"/>
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={data}>
                    ...
                    <Tooltip content={<CustomTooltip />}/>
                    <Legend />
                    <Bar dataKey="cpuUsage" fill="hsl(var(--primary))" name="CPU Usage %" radius={[2, 2, 0, 0]}/>
                    <Bar dataKey="memoryUsage" fill="hsl(var(--secondary))" name="Memory Usage %" radius={[2, 2, 0, 0]}/>
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
