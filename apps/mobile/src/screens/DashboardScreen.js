...
const DashboardScreen = () => {
    ...
    useEffect(() => {
        const unsubscribe = webSocketService.subscribe('alert', (alertData) => {
                ...
                switch (alertData.type) {
                    case 'error':
                        if (settings.notifications.nodeOffline) {
                            notificationService.scheduleNodeOfflineAlert(alertData.nodeId || 'Unknown Node', alertData.nodeId || '');
                        }
                        break;
                    case 'warning':
                        if (settings.notifications.lowPerformance) {
                            notificationService.scheduleLowPerformanceAlert(alertData.nodeId || 'Unknown Node', 75 // This would come from the alert data
                            );
                        }
                        break;
                }
    ...
    return (<ScrollView style={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={handleRefresh}/>}>
      ...
      {/* Alerts */}
      {activeAlerts.map(alert => (<AlertBanner key={alert.id} alert={alert} onDismiss={handleDismissAlert} onPress={handleAlertPress}/>))}
      ...
      <View style={styles.earningsContainer}>
        ...
        <View style={styles.earningsCard}>
          ...
          {isReinvesting && (<View style={styles.reinvestingIndicator}>
      ...
      <View style={styles.nodesSection}>
        ...
        {nodes.slice(0, 3).map(node => (<NodeCard key={node.id} node={node} onPress={() => {
      ...
      {settings.autoReinvest && (<View style={styles.autoReinvestContainer}>
