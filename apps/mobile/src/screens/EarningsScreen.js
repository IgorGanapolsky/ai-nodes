...
const EarningsScreen = () => {
    ...
    const { earnings, chartData, loading, totalEarnings, todayEarnings, yesterdayEarnings, earningsGrowth, refreshEarnings, } = useEarnings(undefined, selectedTimeRange);
    const { triggerReinvest, isReinvesting, reinvestHistory, totalReinvested, successfulReinvests, } = useReinvest();
    ...
    return (<ScrollView style={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={handleRefresh}/>}>
      {/* Summary Cards */}
      ...
      <View style={styles.timeRangeContainer}>
        ...
        <View style={styles.timeRangeSelector}>
          {timeRanges.map(range => (<TouchableOpacity key={range.key} style={[
      ...
      {/* Earnings Chart */}
      {chartData && (<EarningsChart data={chartData} type="line" title="Earnings Over Time" subtitle={`${timeRanges.find(r => r.key === selectedTimeRange)?.label} view`}/>)}
      ...
      <View style={styles.reinvestSection}>
        ...
        <View style={styles.reinvestCard}>
          <View style={styles.reinvestHeader}>
            ...
            <TouchableOpacity style={[styles.reinvestButton, isReinvesting && styles.reinvestButtonDisabled]} onPress={handleManualReinvest} disabled={isReinvesting}>
              {isReinvesting ? (<ActivityIndicator size="small" color="#FFFFFF"/>) : (<Text style={styles.reinvestButtonText}>Reinvest Now</Text>)}
            </TouchableOpacity>
          </View>
      ...
      {reinvestHistory.length > 0 && (<View style={styles.historySection}>
          <Text style={styles.sectionTitle}>Recent Reinvest History</Text>
          {reinvestHistory.slice(0, 5).map((record, index) => (<View key={index} style={styles.historyItem}>
      ...
      <View style={styles.transactionsSection}>
        ...
        {earnings.slice(0, 10).map((earning, index) => (<View key={index} style={styles.transactionItem}>
