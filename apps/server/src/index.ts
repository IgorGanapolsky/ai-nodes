...
async function start() {
  try {
    ...
    process.on('uncaughtException', (error) => {
      app.log.fatal('Uncaught Exception: ' + (error instanceof Error ? error.message : String(error)));
      process.exit(1);
    });

    // Handle unhandled promise rejections
    process.on('unhandledRejection', (reason, promise) => {
      app.log.fatal('Unhandled Rejection at: ' + String(promise) + ' reason: ' + String(reason));
      process.exit(1);
    });
