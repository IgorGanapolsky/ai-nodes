...
const metricRoutes = async (fastify) => {
    ...
    fastify.get('/', {
        ...
        try {
            ...
            let filteredResponse = { summary: mockMetrics.summary };
            if (metricType === 'all') {
...
function generateMockUtilizationData(start, end, granularity, deviceId) {
    const data = [];
    ...
    for (let time = start.getTime(); time <= end.getTime(); time += interval) {
        for (const devId of deviceIds) {
            data.push({
                ...
                deviceId: devId,
                deviceName: deviceNames[devId] || `Device ${devId}`,
...
function generateMockEarningsData(start, end, granularity, deviceId) {
    const data = [];
    const deviceIds = deviceId ? [deviceId] : ['1', '2', '3'];
    const deviceNames = { '1': 'GPU Rig Alpha', '2': 'Storage Node Beta', '3': 'CPU Farm Gamma' };
    ...
    for (let time = start.getTime(); time <= end.getTime(); time += interval) {
        for (const devId of deviceIds) {
            const hourlyRate = rates[devId] || 1.0;
...
function generateMockPerformanceData(start, end, granularity, deviceId) {
    const data = [];
    const deviceIds = deviceId ? [deviceId] : ['1', '2', '3'];
    const deviceNames = { '1': 'GPU Rig Alpha', '2': 'Storage Node Beta', '3': 'CPU Farm Gamma' };
    const interval = granularity === 'hour' ? 60 * 60 * 1000 : 24 * 60 * 60 * 1000;
