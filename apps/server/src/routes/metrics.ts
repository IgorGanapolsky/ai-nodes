...
const metricRoutes: FastifyPluginCallback<{}, any, ZodTypeProvider> = async (fastify) => {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        fastify.log.error(`Error fetching metrics: ${(error as Error).message || String(error)}`);
  ...
  fastify.get('/live', async (_, reply) => {
    try {
      ...
      fastify.log.error(`Error fetching live metrics: ${(error as Error).message || String(error)}`);
      return reply.status(500).send({
        error: 'Internal server error',
  ...
  fastify.get<{
    ...
    async (request, reply) => {
      try {
        ...
        fastify.log.error(`Error exporting metrics: ${(error as Error).message || String(error)}`);
        return reply.status(500).send({
          error: 'Internal server error',
