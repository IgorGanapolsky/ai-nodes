...
const deviceRoutes: FastifyPluginCallback<{}, any, ZodTypeProvider> = async (fastify) => {
  ...
  fastify.get('/:id', async (request, reply) => {
    try {
      ...
      fastify.log.error(error, `Error fetching device ${(request.params as { id: string }).id}`);
  ...
  fastify.put(
    ...
    async (request, reply) => {
      try {
        const { id } = request.params as { id: string };
        const _updates = request.body;
        // Updates are currently unused as this is a mock implementation
        ...
        fastify.log.info({ deviceId: id }, 'Updated device');

        return reply.send(updatedDevice);
      } catch (error) {
        fastify.log.error(error, `Error updating device ${(request.params as { id: string }).id}`);
        return reply.status(500).send({
          error: 'Internal server error',
  ...
  fastify.delete('/:id', async (request, reply) => {
    try {
      ...
      fastify.log.info({ deviceId: id }, 'Deleted device');

      return reply.status(204).send();
    } catch (error) {
      fastify.log.error(error, `Error deleting device ${(request.params as { id: string }).id}`);
      return reply.status(500).send({
        error: 'Internal server error',
