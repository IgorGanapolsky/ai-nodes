...
export class Scheduler {
  ...
  private async runOutreachSender(): Promise<{ processed: number; emailed: number }> {
    ...
    const outreach = (issues || []).filter((i) => i.labels.some((l) => l.name.toLowerCase() === 'outreach'));
    ...
    for (const issue of outreach) {
      ...
      processed++;
      const subject = `Outreach Draft: ${issue.title}`;
      const html = `
        <p>This is an outreach draft generated by the revenue loop.</p>
        <p><b>Linear URL:</b> <a href="${issue.url}">${issue.url}</a></p>
        <p><b>Priority:</b> P${issue.priority}</p>
        <pre style="white-space:pre-wrap;border:1px solid #eee;padding:8px">${(issue.description || '').slice(0, 1200)}</pre>
      ...
      try {
        if (dryRun) {
          ...
          // TODO: resolve real recipient via enrichment; for now require OUTREACH_TEST_EMAIL
          if (toEmail && notifier.isAvailable()) {
            await notifier.send(toEmail, subject, html);
            emailed++;
          }
        ...
        this.app.log.warn(`Failed sending outreach draft for ${issue.id}: ${(e as Error).message}`);
      }
    }
