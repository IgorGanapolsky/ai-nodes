        ...
        try {
            ...
            try {
                ...
                console.log('\n' + createTable([
                    ...
                    { title: 'Change', key: 'priceChange', color: (value) => {
                ...
                const totalPotentialIncrease = filteredSuggestions.reduce((sum, s) => sum + Math.max(0, s.suggestedPrice - s.currentPrice), 0);
                const totalPotentialDecrease = filteredSuggestions.reduce((sum, s) => sum + Math.min(0, s.suggestedPrice - s.currentPrice), 0);
    ...
    return new Command('apply')
        ...
        try {
            ...
            console.log('\n' + createTable([
                ...
                { title: 'Change', key: 'change', color: (value) => {
                        const num = parseFloat(value.replace(/[^-\d.]/g, ''));
                        return num > 0 ? chalk.green(value) : chalk.red(value);
                    } },
                { title: 'Reason', key: 'reason' }
            ...
            // Confirmation
            if (!options.force) {
                const totalRevenueDelta = selectedSuggestions.reduce((sum, s) => sum + (s.suggestedPrice - s.currentPrice), 0);
            ...
            try {
                ...
                // Show summary
                const totalIncrease = selectedSuggestions.reduce((sum, s) => sum + Math.max(0, s.suggestedPrice - s.currentPrice), 0);
                const totalDecrease = selectedSuggestions.reduce((sum, s) => sum + Math.min(0, s.suggestedPrice - s.currentPrice), 0);
        ...
        try {
            ...
            try {
                ...
                console.log('\n' + createTable([
                    ...
                    { title: 'Performance', key: 'performance', color: (value) => {
