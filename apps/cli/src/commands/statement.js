...
function displayStatement(statement, owner) {
    ...
    const averageUtilization = statement.devices.reduce((sum, d) => sum + (d.metrics?.utilization || 0), 0) / Math.max(totalDevices, 1);
...
function generateCSV(statement, owner) {
    ...
    statement.devices.forEach(device => {
        lines.push(`${device.name},${device.type},${device.status},${device.metrics?.grossRevenue24h || 0},${device.metrics?.utilization || 0}`);
    });
    return lines.join('\n');
}
