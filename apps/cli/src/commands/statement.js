  ...
  return new Command('generate')
    ...
    .action(async (options) => {
      try {
        ...
        console.error(
          chalk.red('Error generating statement:'),
  ...
  return new Command('list')
    ...
    .action(async (options) => {
      try {
        ...
        console.log(
          'This would show previously generated statements with dates, owners, and status.',
        );
      } catch (error) {
        console.error(
          chalk.red('Error listing statements:'),
          error instanceof Error ? error.message : error,
        );
        process.exit(1);
      }
    ...
    .action(async (options) => {
      try {
        ...
        try {
          ...
          const totalDailyRevenue = devices.reduce(
            (sum, d) => sum + (d.metrics?.grossRevenue24h || 0),
            0,
          );
          const totalWeeklyRevenue = devices.reduce(
            (sum, d) => sum + (d.metrics?.grossRevenue7d || 0),
            0,
          );
          ...
          const ownerSummaries = owners.map((owner) => {
            ...
            const ownerDailyRevenue = ownerDevices.reduce(
              (sum, d) => sum + (d.metrics?.grossRevenue24h || 0),
              0,
            );
            const ownerWeeklyRevenue = ownerDevices.reduce(
              (sum, d) => sum + (d.metrics?.grossRevenue7d || 0),
              0,
            );
            return {
              ...
              averageDeviceRevenue:
                ownerDevices.length > 0 ? ownerDailyRevenue / ownerDevices.length : 0,
            };
          });
          ...
          console.log(
            '\n' +
              createKeyValueTable({
                ...
                'Average Revenue per Device': formatters.currency(
                  totalDailyRevenue / Math.max(totalDevices, 1),
                ),
                'Network Utilization': formatters.percentage(
                  devices.reduce((sum, d) => sum + (d.metrics?.utilization || 0), 0) /
          ...
          const revenueDistribution = ownerSummaries
            .map((s) => s.dailyRevenue)
            .sort((a, b) => b - a);
          const topPerformer = ownerSummaries.find(
            (s) => s.dailyRevenue === revenueDistribution[0],
          );
          const median = revenueDistribution[Math.floor(revenueDistribution.length / 2)] || 0;
          console.log('\n' + chalk.bold.cyan('📊 Revenue Distribution Analysis:'));
          console.log(
            `• Top performing owner: ${topPerformer?.owner.name || 'N/A'} (${formatters.currency(revenueDistribution[0] || 0)}/day)`,
          );
          console.log(`• Median owner revenue: ${formatters.currency(median)}/day`);
          console.log(
            `• Revenue concentration: Top owner represents ${(((revenueDistribution[0] || 0) / totalDailyRevenue) * 100).toFixed(1)}% of total revenue`,
          );
          // Growth projections
          const weeklyGrowthRate =
            totalWeeklyRevenue > 0 ? ((totalDailyRevenue * 7) / totalWeeklyRevenue - 1) * 100 : 0;
          console.log(
            `• Weekly growth rate: ${weeklyGrowthRate >= 0 ? '+' : ''}${weeklyGrowthRate.toFixed(1)}%`,
          );
          // Recommendations
          ...
          if (inactiveOwners.length > 0) {
            recommendations.push(
              `${inactiveOwners.length} owners have no revenue - provide onboarding support`,
            );
          }
        ...
        console.error(
          chalk.red('Error generating summary:'),
          error instanceof Error ? error.message : error,
        );
        process.exit(1);
      }
...
function displayStatement(statement, owner) {
  // Header
  ...
  const averageUtilization =
    statement.devices.reduce((sum, d) => sum + (d.metrics?.utilization || 0), 0) /
  ...
  console.log(
    `• Revenue per device: ${formatters.currency(statement.totalRevenue / Math.max(totalDevices, 1))}`,
  );
  if (statement.netProfit > 0) {
    console.log(
      `• ROI: Profitable operation with ${formatters.percentage((statement.netProfit / Math.max(statement.totalExpenses, 1)) * 100)} return`,
    );
  } else {
    console.log(`• ROI: ${chalk.red('Operating at a loss')} - consider optimization strategies`);
  }
}
function generateCSV(statement, owner) {
  const lines = [
    'Financial Statement',
    `Owner,${owner.name}`,
    `Email,${owner.email}`,
